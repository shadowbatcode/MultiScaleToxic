...
from .transformersv2 import TransformerEncoderWithPairV2,FeatureFusion
from .ESMAA import ESM2_AA_Toxicity
from ..utils import pad_1d_tokens, pad_2d, pad_coords,init_weights
import argparse
import pathlib
import os

from.compar_learing import RelationalContrast
...
BACKBONE = {
    ...
    'esm': ESM2_AA_Toxicity
}

class ToxMolModel(nn.Module):
    """
    ...
    def __init__(self, output_dim=2, model_size='84m', **params):
        ...
        name = model_size
        self.pretrain_path = os.path.join(WEIGHT_DIR, "modelzoo/best/checkpoint.pth")
        ...
        self.encoder_norm = nn.LayerNorm(self.args.encoder_embed_dim)
        self.resident_norm = nn.LayerNorm(self.args.resident_dim)
        #self.resident_encoder = BACKBONE['esm'](embed_dim=3140)
        self.contrastive = RelationalContrast(hidden_dim=self.args.encoder_embed_dim+self.args.resident_dim)
        self.load_pretrained_weights(path=self.pretrain_path,strict=False)
        self.atom_feature.apply(init_weights)
        self.comparative_learning = params['comparative_learning']
        self.indexs = [22,23,108,111,132,142,143,144,146,147,151,163,398] #获取AAindex中具有价值的部分
        if self.comparative_learning:
            self.fusion_location = params['fusion_location']
            if self.fusion_location == 'after':
                self.feature_fusion = FeatureFusion(self.args.encoder_embed_dim,len(self.indexs))
                #self.contrast_module = MultiModalContrast(self.args.encoder_embed_dim)
            elif 'layer' in self.fusion_location:
                self.fusion_layer = int(self.fusion_location[-1])
                self.activations = {}
                def get_activation(name):
                    def hook(module, input, output):
                        self.activations[name] = input[0].detach()
                    return hook
                self.encoder.layers[self.fusion_layer].register_forward_hook(
                    get_activation('fusion_layer'))
        else:
            self.fusion_location = ''
        ...
        if path is not None:
            ...
            try:
                ...
                if 'classification_head.dense.weight' in state_dict:
                    self.classification_head = ClassificationHead(
                        input_dim=self.args.encoder_embed_dim,#+self.args.resident_dim,
                        inner_dim=self.args.encoder_embed_dim,
                        num_classes=self.output_dim,
                        activation_fn=self.args.pooler_activation_fn,
                        pooler_dropout=self.args.pooler_dropout,
    ...
    def calc_contrast(self, x, residue_feat, adjacency):
        """对比损失计算"""
        if 'layer' in self.fusion_location:
            device = torch.device("cuda:1" if torch.cuda.is_available() and self.cuda else "cpu")
            fusion_feat = self.activations['fusion_layer']
            fusion_feat = fusion_feat[:, 1:, :]#.to(device)
            #residue_feat = residue_feat.to(device)
            #adjacency = adjacency.to(device)
            #return self.contrastive(fusion_feat, residue_feat, adjacency)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.fusion_location == 'before': 
            x = self.atom_feature({'atom_feat': atom_feat, 'degree': degree}, token_feat,res_features=residue)
            adjacency = (torch.cdist(pos, res_coords) < 8.0).float()
            self.contrast_loss = self.calc_contrast(x, residue, adjacency)
        else:
        ...
        def one_block(x, pos, return_x=False):
            ...
            x, pair = self.encoder(
                ...
                pair_mask=pair_mask,
                attn_mask=attn_mask,
                residue_feature = residue,
                fusion_position = self.fusion_location
            )
            
            if self.comparative_learning and 'layer' in self.fusion_location:
                adjacency = (torch.cdist(pos, res_coords) < 8.0).float()
                self.contrast_loss = self.calc_contrast(x, residue, adjacency)
        ...
        if self.comparative_learning:
            if self.fusion_location == 'after':
                # 在特征融合前添加
                atom_feature = self.encoder_norm(x)
                residue = self.resident_norm(residue)
                aaindex = residue[:,:,2560:][:,:,self.indexs]
                x = self.feature_fusion(atom_feat=atom_feature, res_feat=aaindex)
                cls_repr = x[:,0,:]
                residue_repr = residue  # 可以考虑引入残基特征压缩模块
                atomic_repr = x[:,1:,:]    # 去除CLS token的原子特征
                adjacency = (torch.cdist(pos, res_coords) < 8.0).float()
                self.contrast_loss = self.calc_contrast(x, residue, adjacency)
            logits = self.classification_head(cls_repr)
            return logits, self.contrast_loss
        else:
            logits = self.classification_head(cls_repr)
            return logits
    ...
    def batch_collate_fn(self, samples):
        ...
        for k in samples[0][0].keys():
            if k == 'atom_feat':
                ...
                v = pad_coords([torch.tensor(s[0][k]) for s in samples], pad_idx=self.padding_idx,dim=3140)
            elif k == 'res_coords':
                v = pad_coords([torch.tensor(s[0][k]) for s in samples], pad_idx=self.padding_idx)
            batch[k] = v
        try:
