...
class FeatureFusion(nn.Module):
    def __init__(self, atom_dim, res_dim):
        super().__init__()
        # 维度对齐层（保留原有结构）
        self.dim_align = nn.Linear(res_dim, atom_dim) if res_dim != atom_dim else nn.Identity()
        
        # 交叉注意力层（增加注意力mask的dropout）
        self.cross_attn = nn.MultiheadAttention(
            embed_dim=atom_dim,
            num_heads=8,
            batch_first=True,
            dropout=0.1  # 新增注意力dropout
        )
        
        # 改进1：门控融合机制
        self.gate_controller = nn.Sequential(
            nn.Linear(atom_dim * 2, atom_dim // 2),
            nn.GELU(),
        ...
        # 改进3：梯度控制包装器
        class PartialGrad(torch.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                return x
            @staticmethod 
            def backward(ctx, grad):
                return grad * 0.4  # 限制梯度回传比例

        self.grad_wrapper = PartialGrad.apply

        # 改进4：动态缩放参数
        self.fusion_alpha = nn.Parameter(torch.tensor(0.3))  # 可学习初始权重

    def forward(self, atom_feat, res_feat):
        # 原始特征梯度保护
        protected_atom_feat = self.grad_wrapper(atom_feat)
        
        # 特征对齐
        aligned_res = self.dim_align(res_feat)
        # 带mask的交叉注意力
        adapted_res, _ = self.cross_attn(
            query=protected_atom_feat,
            key=aligned_res,
            value=aligned_res,
            need_weights=False
        )
        ...
        # 改进适配器处理
        processed_res = self.res_adapter(adapted_res)
        
        # 门控融合机制
        gate = self.gate_controller(
            torch.cat([protected_atom_feat, processed_res], dim=-1)
        )
        fused_feat = protected_atom_feat + self.fusion_alpha * (gate * processed_res)
        
        # 层归一化稳定输出
        return nn.LayerNorm(fused_feat.shape[-1]).to(fused_feat.device)(fused_feat)
...
class AtomFeature(nn.Module):
    ...
    def __init__(
        ...
        self.fusion = FeatureFusion(hidden_dim, hidden_dim)
        self.esm_proj = nn.Linear(50, hidden_dim // 2)
        self.aaindex_proj = nn.Linear(100, hidden_dim // 2)
    ...
    def forward(self, batched_data, token_feat,res_features=None):
        x, degree = (
        ...
        if res_features is not None:
            esm_feat = self.esm_proj(res_features[..., :50])  # [n_graph, n_node, h/2]
            aaindex_feat = self.aaindex_proj(res_features[..., 50:])  # [n_graph, n_node, h/2]
            combined_res = torch.cat([esm_feat, aaindex_feat], dim=-1)  # [n_graph, n_node, h]
            node_feature = self.fusion(node_feature, combined_res)
