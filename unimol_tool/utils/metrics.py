...
METRICS_REGISTER = {
    'regression': {
        ...
        "rmse": [rmse, False, 'float'],
        "r2": [r2_score, True, 'float'],
        "sd": [lambda y_true, y_pred: np.std(y_true - y_pred), False, 'float'],  # SD of errors
        "ci": [lambda y_true, y_pred: scipy.stats.t.interval(0.95, len(y_true)-1, loc=np.mean(y_pred - y_true), scale=scipy.stats.sem(y_pred - y_true)),False,'tuple']
    },
    'classification': {
...
DEFAULT_METRICS = {
    ...
    'classification': ['log_loss', 'auc','auprc', 'f1_score', 'mcc', 'acc', 'precision', 'recall'],
