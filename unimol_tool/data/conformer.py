...
class UniMolV2Feature(object):
    ...
    def transform_raw(self, atoms_list, coordinates_list, residue_list=[],residue_coord=[]):
            
            inputs = []
            index = 0
            for atoms, coordinates in zip(atoms_list, coordinates_list):
                if len(residue_list)>index:
                    residue = np.array(residue_list[index])
                    rescoord = np.array(residue_coord[index])
                    #print(f"原始:{coordinates.shape};{rescoord.shape}")
                else:
                    residue = None
                    rescoord = None
                mol = create_mol_from_atoms_and_coords(atoms, coordinates)
                inputs.append(mol2unimolv2(mol, self.max_atoms, remove_hs=self.remove_hs,residue_feats=residue,res_coords=rescoord))
                index += 1
            return inputs
...
def mol2unimolv2(mol, max_atoms=128, remove_hs=True,residue_feats=None,res_coords=None, **params):
    """
    Converts atom symbols and coordinates into a unified molecular representation.
    ...
    post_h_removal_indices = [atom.GetIdx() for atom in mol.GetAtoms()]
    ...
    if isinstance(residue_feats,np.ndarray) or isinstance(residue_feats,list):

        updated_residue = residue_feats[post_h_removal_indices, :]
        updated_residue_coord = res_coords[post_h_removal_indices, :]
        if 'mask' in locals():
            updated_residue = updated_residue[mask]
            updated_residue_coord = updated_residue_coord[mask]
        feat['residue'] = updated_residue 
        feat['res_coords'] = updated_residue_coord.astype(np.float32)
        #print(f"完整shape:{np.array(src_tokens).shape},{updated_residue.shape};{src_coord.shape},{updated_residue_coord.shape};")
...
def get_graph_features(edge_attr, edge_index, node_attr, drop_feat, mask):
    ...
    shortest_path_result = floyd_warshall(adj) #数据量庞大会卡顿:floyd_warshall(adj)
    # shortest_path_result = np.nan_to_num(shortest_path_result, nan=510)  # 处理不可达路径
    # shortest_path_result = shortest_path_result.astype(np.int64)  # 强制转为整数 <--- 关键步骤
    # shortest_path = torch.from_numpy(shortest_path_result).cuda()
    # max distance is 509
    if drop_feat:
        atom_feat[...] = 1
        edge_feat[...] = 1
