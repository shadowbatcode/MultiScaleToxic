...
class DataHub(object):
    ...
    def _init_data(self, **params):
        ...
        elif params.get('model_name', None) == 'toxmol':
            residue_feats = self.data['residues']
            residue_coord = self.data['res_coords']
            if 'atoms' in self.data and 'coordinates' in self.data:
                no_h_list = UniMolV2Feature(**params).transform_raw(self.data['atoms'], self.data['coordinates'],residue_feats,residue_coord)
            else:
                smiles_list = self.data["smiles"]                  
                no_h_list = UniMolV2Feature(**params).transform(smiles_list)
