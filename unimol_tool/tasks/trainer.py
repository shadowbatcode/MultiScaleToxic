...
class Trainer(object):
    ...
    def _init_trainer(self, **params):
        ...
        self.comparative_learning = params.get('comparative_learning',False)
    ...
    def decorate_torch_batch(self, batch):
        ...
        if self.task == 'repr':
            net_target = net_target.long() #None
    ...
    def fit_predict(self, model, train_dataset, valid_dataset, loss_func, activation_fn, dump_dir, fold, target_scaler, feature_name=None):
        ...
        # optimizer = AdamW(
        #     [
        #         # 第一组：预训练参数（低学习率，高权重衰减）
        #         {
        #             'params': [p for p in model.parameters() if not hasattr(p, '_is_new_param')],
        #             'lr': 1e-5,  # 建议1e-5
        ...
        #         # 第二组：新增参数（高学习率，无权重衰减）
        #         {
        #             'params': [p for p in model.parameters() if hasattr(p, '_is_new_param')],
        #             'lr': 3e-4, # 建议3e-4 
        #             'weight_decay': 0.0,      # 避免抑制新特征学习
        #             'eps': 1e-6               # 保持数值稳定性
        #         }
        #     ],
        #     # 全局参数（如有需要可覆盖）
        #     lr=self.learning_rate,  # 作为fallback值
        #     eps=1e-6
        # )
        optimizer = AdamW(model.parameters(), lr=self.learning_rate, eps=1e-6)
            ...
            for i, batch in enumerate(train_dataloader):
                ...
                if self.scaler and self.device.type == 'cuda':
                    with torch.cuda.amp.autocast():
                        if not self.comparative_learning:
                            outputs,data = model(**net_input)
                            loss = loss_func(outputs, net_target)
                        else:
                            outputs,contrast_loss = model(**net_input,label=net_target)
                            contrast_loss = 0#contrast_loss.mean()
                            loss = loss_func(outputs, net_target)
                            #multrn_loss.append(float(contrast_loss.data))
                else:
                ...
                if self.comparative_learning:
                    current_step = i % self.max_epochs  # 每1000步调整一次
                    alpha = 0.1 * (1 + np.cos(current_step/self.max_epochs * np.pi))  # 余弦波动权重 [0, 0.2]
                    #loss = alpha * contrast_loss + (1-alpha) * loss
                    batch_bar.set_postfix(
                        Epoch="Epoch {}/{}".format(epoch+1, self.max_epochs),
                        total_loss="{:.04f}".format(loss),
                        loss="{:.04f}".format(float(sum(trn_loss) / (i + 1))),
                        contrast_loss="{:.04f}".format(float(sum(multrn_loss) / (i + 1))),
    ...
    def predict(self, model, dataset, loss_func, activation_fn, dump_dir, fold, target_scaler=None, epoch=1, load_model=False, feature_name=None):
        ...
        if load_model == True:
            ...
            model.module.load_pretrained_weights(load_model_path, strict=False)
            ...
            if not load_model:
                if not self.comparative_learning:
                    ...
                    batch_bar.set_postfix(
                        Epoch="Epoch {}/{}".format(epoch+1, self.max_epochs),
                        loss="{:.04f}".format(float(np.sum(val_loss) / (i + 1))),
                        contrast_loss="{:.04f}".format(float(np.sum(multi_loss) / (i + 1)))
                        )                    
            batch_bar.update()
    ...
    def inference(self, model, dataset, return_repr=False, return_atomic_reprs=False, feature_name=None):
        ...
        model = model.eval()
        repr_dict = {"cls_repr": [], "atomic_coords": [], "atomic_reprs": [], "atomic_symbol": [],"label":[]}
            ...
            with torch.no_grad():
                ...
                if return_atomic_reprs:
                    ...
                    if outputs.get("label"):
                        repr_dict["label"].extend(outputs["label"])
                    else:
                        print(net_target)
                        repr_dict["label"].extend(item.cpu().numpy() for item in net_target)
